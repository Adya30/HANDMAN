...
<div class="space-y-6 pb-10">
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-2 shrink-0">
        <div>
            ...
            <p class="text-sm text-gray-500 mt-0.5">Selamat datang kembali, <span class="font-semibold text-gray-700">{{ Auth::user()->nama_lengkap }}</span>. Berikut ringkasan tugas Anda hari ini.</p>
        </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 shrink-0">
        ...
        <div class="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex items-center justify-between">
            <div class="space-y-1">
                ...
                <p class="text-[10px] text-gray-400">Tugas didelegasikan</p>
            </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 shrink-0">
        <div class="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm flex items-center gap-3">
            ...
            <div>
                <p class="text-[10px] font-medium text-gray-400 uppercase tracking-wider">Grup Kerja Saya</p>
                <p class="text-xl font-bold text-gray-800">{{ $totalGrupSaya }}</p>
            </div>
        </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <div>
                    <h2 class="text-sm font-bold text-gray-900">Laporan Masalah Saya</h2>
                    <p class="text-xs text-gray-400 mt-0.5">Status kendala yang telah Anda laporkan.</p>
                </div>
                <a href="{{ route('staff.laporan.index') }}" class="text-xs font-semibold text-[#3B28CC] hover:underline">Lihat semua →</a>
            </div>
            <div class="divide-y divide-gray-50">
                ...
                <div class="px-5 py-3.5 flex items-center gap-4 hover:bg-gray-50/60 transition-colors">
                    ...
                    <div class="flex-1 min-w-0">
                        ...
                        <p class="text-[10px] text-gray-400">Dilaporkan {{ $laporan->created_at->diffForHumans() }}</p>
                    </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-0.5">5 tugas didelegasikan paling baru.</p>
                </div>
                <a href="{{ route('staff.tugas.index') }}" class="text-xs font-semibold text-[#3B28CC] hover:underline">Lihat semua →</a>
            </div>
            <div class="divide-y divide-gray-50">
                @forelse($tugas as $t)
                @php
                ...
                <div class="px-5 py-3.5 flex items-center gap-3 hover:bg-gray-50/60 transition-colors">
                    <div class="flex-1 min-w-0">
                        <a href="{{ route('staff.tugas.show', $t->id) }}" class="text-sm font-semibold text-gray-800 truncate hover:text-[#3B28CC] block">
                            {{ $t->nama_tugas }}
                        </a>
                ...
                @empty
                <div class="px-5 py-8 text-center text-xs text-gray-400">Belum ada tugas didelegasikan.</div>
                @endforelse
            </div>
        </div>
    </div>
